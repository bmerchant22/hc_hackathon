...
import (
	...
	"github.com/bmerchant22/hc_hackathon/ws"
...
func CreateAppointmentHandler(ctx *gin.Context) {
	...
	var doctor auth.User
	if err := db.Where("user_id = ?", appointment.DocId).First(&doctor).Error; err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "DocId not found"})
		return
	}

	if err := ws.Notify(appointment, &doctor); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to forward appointment to the doctor"})
		return
	}
